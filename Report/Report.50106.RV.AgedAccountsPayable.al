/// <summary>
/// Report RV Aged Accounts Payable Basic (ID 50106).
/// FDD030 2026/06/01: New. (Liu.yang)
/// Custom version of standard BC Aged Accounts Payable report.//report 322 "Aged Accounts Payable"
/// </summary>
report 50106 "RV Aged Accounts Payable Basic"
{
    ApplicationArea = All;
    Caption = 'RV Aged Accounts Payable';
    UsageCategory = ReportsAndAnalysis;
    DataAccessIntent = ReadOnly;
    //DefaultLayout = RDLC;
    //RDLCLayout = './ReportLayout/RV_AgedAccountsPayable.rdlc';
    DefaultRenderingLayout = "RV_AgedAccountsPayableSummary.rdlc";
    ...
    dataset
    {
        dataitem(Vendor; Vendor)
        {
            ...
            column(VendNoFilter; VendNoFilter) { }
            column(CurrencyFilter; CurrencyFilter) { }
            column(VendPayTerm; VendPayTerm) { }
            column(VendCurrency; VendCurrency) { }
            column(VendBankNo; VBActNo) { }
            column(VendBankName; VBActName) { }
            column(VendBankSWIFT; VBActSWIFT) { }
            column(PrintBankDtl; PrintBankDtl) { }
            ...
            dataitem(CurrencyLoop; "Integer")
            {
                ...
                dataitem(TempVendortLedgEntryLoop; "Integer")
                {
                    ...
                    begin
                        ...
                        CurrExchRate.SetRange("Currency Code", VendorLedgEntryEndingDate."Currency Code");
                        CurrExchRate.SetRange("Starting Date", 0D, VendorLedgEntryEndingDate."Posting Date");
            ...
            begin
                ...
                VendCurrency := Vendor."Currency Code";
                VendPayTerm := Vendor."Payment Terms Code";
                ...
                if VendBankAct.FindSet() then begin
                    ...
                    VBActSWIFT := VendBankAct."SWIFT Code";
                end;
            end;
        ...
        dataitem(CurrencyTotals; "Integer")
        {
            ...
            column(AgedVendLedgEnt7RemAmtLCY5; AgedVendorLedgEntry[7]."Remaining Amount")
            {
                AutoFormatExpression = TempCurrency2.Code;
                AutoFormatType = 1;
            }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(PrintBankDtl; PrintBankDtl)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Print Bank Details';
                        ToolTip = 'Specifies if you want the report to show the Bank details that add up the total balance for each vendor.';
                    }
    ...
    rendering
    {
        layout("RV_AgedAccountsPayableSummary.rdlc")
        {
            Type = RDLC;
            LayoutFile = '.\ReportLayout\RV_AgedAccountsPayableSummary.rdlc';
    ...
    begin
        ...
        VendNoFilter := Vendor.GetFilter("No.");
        CurrencyFilter := Vendor.GetFilter("Currency Code");

        if VendNoFilter.Trim() = '' then
            VendNoFilter := 'ALL';
        if CurrencyFilter.Trim() = '' then
            CurrencyFilter := 'ALL';

        VendNoFilter := 'Trading Partner From - To: ' + VendNoFilter;
        CurrencyFilter := 'Currency: ' + CurrencyFilter;

        if PrintDetails then
            AgedAcctPayableCaptionLbl := 'ACCOUNT PAYABLE AGING REPORT'
        else
            AgedAcctPayableCaptionLbl := 'ACCOUNT PAYABLE AGING SUMMARY REPORT';
    end;
    ...
    var
        ...
        AgedVendorLedgEntry: array[7] of Record "Vendor Ledger Entry";
...
#pragma warning restore AA0074
        ...
        AgedAcctPayableCaptionLbl: Text;//Label 'ACCOUNT PAYABLE AGING SUMMARY REPORT';
        ...
        VendBankAct: Record "Vendor Bank Account";
        VBActName: Text;
        VBActSWIFT: Text;
        VBActNo: Text;
    ...
    begin
        ...
        if HeadingType = HeadingType::"Number of Months" then HeaderText[1] := 'Current';

        while i < ArrayLen(PeriodEndDate) do begin
            if HeadingType = HeadingType::"Date Interval" then
                HeaderText[i] := StrSubstNo('%1\..%2', PeriodStartDate[i], PeriodEndDate[i])
            else if HeadingType = HeadingType::"Number of Months" then
