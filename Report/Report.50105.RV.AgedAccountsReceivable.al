...
report 50105 "RV Aged Accounts Receivable"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './ReportLayout/RV_AgedAccountsReceivable.rdlc';
    Caption = 'RV Aged Accounts Receivable';
    ...
    begin
        ...
        CustNoFilter := Customer.GetFilter("No.");
        CurrencyFilter := Customer.GetFilter("Currency Code");

        if CustNoFilter.Trim() = '' then
            CustNoFilter := 'ALL';
        if CurrencyFilter.Trim() = '' then
            CurrencyFilter := 'ALL';

        CustNoFilter := 'Trading Partner From - To: ' + CustNoFilter;
...
#pragma warning restore AA0074
        ...
        AgedAccReceivableCptnLbl: Label 'ACCOUNT RECEIVABLE AGING REPORT';
